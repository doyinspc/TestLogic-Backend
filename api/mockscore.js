const  { MOCKSCORES } =  require('./types');
...
router.get(`/cloud/:${TABLE[1]}`, (req, res)=>{
    //let id = res.params.mockscoreID;
    const sql = `SELECT * FROM mockscores JOIN themes ON mockscores.themeID = themes.id WHERE themes.subjectID = 1`;
    console.log(sql);
    request(sql, res);
})
